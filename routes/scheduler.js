...
router.post('/searchScheduler', function (req, res) {
    var mysqlConn = req.mysqlConn;
    var searchWords = mysqlConn.escape(req.body.searchWords);
    var cur_page = mysqlConn.escape(req.body.cur_page) * 1;
    var per_page = mysqlConn.escape(req.body.per_page) * 1;
    async.parallel([
        function (callback) {
            var query = "select * from scheduleMaster sm, batchMaster bm where sm.batch_id = bm.batch_id and sm.batch_args like concat('%', " + searchWords + ", '%') ";
            query += "LIMIT " + per_page + " OFFSET " + ((cur_page - 1) * per_page) + " ";
//            query = query.replace(/\\'/g, "'");
            ...
            var query = "select count(*) as cnt from scheduleMaster where batch_args like concat('%', " + searchWords + ", '%') ";
//            query = query.replace(/\\'/g, "'");
            mysqlConn.query(query, function (err, cnt) {
                callback(null, cnt);
            });
