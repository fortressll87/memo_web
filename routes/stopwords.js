...
router.post('/searchStopwords', function (req, res) {
    var mysqlConn = req.mysqlConn;
    ...
    async.parallel([
        function (callback) {
            var query = "select * from stopwords where name like concat('%', " + searchWords + ", '%') ";
            query += "LIMIT " + per_page + " OFFSET " + ((cur_page - 1) * per_page) + " ";
//            query = query.replace(/\\'/g, "'");
            ...
            var query = "select count(*) as cnt from stopwords where name like concat('%', " + searchWords + ", '%') ";
//            query = query.replace(/\\'/g, "'");
            mysqlConn.query(query, function (err, cnt) {
                callback(null, cnt);
            });
